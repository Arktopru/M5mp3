...
#include <FS.h>
#include <Wire.h>
#include <SD.h>           // version 1.3.0
#include "M5Cardputer.h"  // version 1.0.3
#include "Audio.h"        // https://github.com/schreibfaul1/ESP32-audioI2S   version 3.0.13
#include "font.h"
...
void setup() {
  ...
  listFiles(SD, rootDir, scanLevel);
...
void listFiles(fs::FS &fs, const char *dirname, uint8_t levels) {
  ...
  while (file && fileCount < MAX_FILES) {
    ...
    if (file.isDirectory()) {
      ...
      if (levels > 0) {
        String directory = String(dirname) + String(file.name()) + "/";
        listFiles(fs, directory.c_str(), levels - 1);
      }
    } else {
      Serial.print(" FILE: ");
      Serial.print(file.name());
      Serial.print(" ");
      audioFiles[fileCount] = dirname + String(file.name());
      fileCount++;
    }
